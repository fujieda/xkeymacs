...
{
	if (imeState)
		CUtils::SetIMEState(true);
	else {
		CUtils::SetIMEState(false);
		CUtils::SetAppName();
	}
	if (_tcsnicmp(m_Config.szSpecialApp[m_nAppID], CUtils::GetAppName(), 0xF) || !IsMatchWindowText(m_Config.szWindowText[m_nAppID])) {	// PROCESSENTRY32 has only 0xF bytes of Name
		m_nAppID = -1;
		for (int nAppID = 0; nAppID < MAX_APP; ++nAppID) {
			if (_tcsnicmp(m_Config.szSpecialApp[nAppID], CUtils::GetAppName(), 0xF) || !IsMatchWindowText(m_Config.szWindowText[nAppID]))
				continue;
			if (m_nAppID < 0)
				m_nAppID = nAppID;
			else {
